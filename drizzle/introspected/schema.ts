import { pgSchema, pgEnum, pgTable, uuid, text, varchar, bigint, integer, bigserial, timestamp, json, jsonb, customType, boolean, inet, smallint, index, uniqueIndex, foreignKey, primaryKey, unique, check, doublePrecision, numeric } from "drizzle-orm/pg-core"
...
export const projectStatus = pgEnum("project_status", ["backlog", "active", "completed"])
...
export const checklistItems = pgTable.withRLS("checklist_items", {
	id: uuid().defaultRandom().primaryKey(),
...
export const checklists = pgTable.withRLS("checklists", {
	id: uuid().defaultRandom().primaryKey(),
...
export const notes = pgTable.withRLS("notes", {
	id: uuid().defaultRandom().primaryKey(),
	userId: uuid("user_id").notNull().references(() => profiles.id, { onDelete: "cascade" } ),
	title: text().notNull(),
	content: text().notNull(),
...
export const snippets = pgTable.withRLS("snippets", {
	id: uuid().defaultRandom().primaryKey(),
	userId: uuid("user_id").notNull().references(() => profiles.id, { onDelete: "cascade" } ),
	title: text().notNull(),
	content: text().notNull(),
